...
def change_client(client_id, name=None, surname=None, email=None):
    ...
    for key, value in atributes_client.items():
        if value:
            with conn.cursor() as cur:
                cur.execute(
                    SQL("UPDATE clients SET {} = %s WHERE id = %s").format(
                        Identifier(key)
                    ),
                    (value, client_id),
                )
...
def delete_phone(client_id, number):
    with conn.cursor() as cur:
        ...
        if len(cur.fetchall()) == 0:
            print(
                "Записи номера клиента с таким телефоном не существует. Проверьте данные."
            )
            return
    with conn.cursor() as cur:
        cur.execute(
...
if __name__ == "__main__":
    with psycopg2.connect(
        database="clients_db", user="postgres", password="2573544"
    ) as conn:
        create_db(conn)
